...
async function findAll({ module_id, project_id, search } = {}) {
  ...
  const res = await pool.query(`${BASE} WHERE ${conds.join(" AND ")} ORDER BY tc.id ASC`, params);
  return res.rows;
}

async function findById(id) {
  const res = await pool.query(`${BASE} WHERE tc.id=$1`, [id]);
  return res.rows[0];
}
...
async function update(id, { module_id, title, description, preconditions, steps, expected_result, priority, assigned_to_id }) {
  await pool.query(`
    UPDATE test_cases SET module_id=$1,title=$2,description=$3,preconditions=$4,steps=$5,expected_result=$6,priority=$7,assigned_to_id=$8
    WHERE id=$9
...
async function remove(id) {
  const res = await pool.query("DELETE FROM test_cases WHERE id=$1", [id]);
