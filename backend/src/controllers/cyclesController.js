...
const storage = multer.diskStorage({
  destination: (_req,_file,cb) => cb(null, process.env.QA_UPLOAD_DIR || "uploads"),
...
const update  = async (req,res,next) => { try { if(!req.body.name?.trim()) return r.badRequest(res,"name obrigatório"); r.ok(res, await svc.updateCycle(req.params.id,req.body)); } catch(e){next(e);} };
...
module.exports = { index, show, store, update, destroy, listExecutions, addExecutions, updateExecution, removeExecution, uploadEvidence, deleteEvidence };
