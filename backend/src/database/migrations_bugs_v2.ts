...
export async function upgradeBugsTable(): Promise<void> {
  if (USE_PG) {
    await query(`
    ...
    await execute(`CREATE TABLE IF NOT EXISTS bug_relations (id INTEGER PRIMARY KEY AUTOINCREMENT, bug_id INTEGER NOT NULL, related_bug_id INTEGER NOT NULL, UNIQUE(bug_id, related_bug_id))`, []).catch(()=>{});
    await execute(`CREATE TABLE IF NOT EXISTS bug_activity (id INTEGER PRIMARY KEY AUTOINCREMENT, bug_id INTEGER NOT NULL, user_id INTEGER, action TEXT NOT NULL, detail TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')))`, []).catch(()=>{});
  }
