...
async function ins(sql: string, params: unknown[]): Promise<number> {
  if (USE_PG) { const r = await query<{id: number}>(sql, params); return r[0]?.id; }
  else { const r = await execute(sql.replace(/\$\d+/g, "?"), params); return r.lastInsertRowid as number; }
}

export async function runSeed(): Promise<void> {
  const rows = await query<{c: string}>("SELECT COUNT(*) AS c FROM users", []);
  const count = USE_PG ? parseInt(rows[0].c) : (rows[0] as any).c;
  if (count > 0) return;
  ...
  for (let i = 0; i < tcs.length; i++) {
    try { await ins("INSERT INTO test_executions (cycle_id,test_case_id,status,executed_by_id) VALUES ($1,$2,$3,$4) RETURNING id", [cycleId, tcs[i], statuses[i], editorId]); }
    catch(_) {}
  }
