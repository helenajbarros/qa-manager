...
import { Loading, ErrorMsg, Empty, Modal, ConfirmModal, Field, Select, Severity, BugStatus } from "../components/UI.js";
import type { Bug, Module, TestCase, Cycle, MentionUser } from "../types/index.js";
...
interface BugFormProps {
  initial?: Partial<Bug & { tracker_url?: string; pr_url?: string }>;
  modules: Module[];
  testCases: TestCase[];
  cycles: Cycle[];
  users: MentionUser[];
  onSave: (form: BugFormData) => void;
  onCancel: () => void;
  saving: boolean;
  bugId?: number;
  onFileUpload?: (bugId: number, file: File) => void;
  onFileDelete?: (bugId: number, fileId: number) => void;
}

type ModalState = { mode: "create"; item?: null } | { mode: "edit"; item: Bug };
...
function BugForm({ initial={}, modules, testCases, users, onSave, onCancel, saving, bugId, onFileUpload, onFileDelete, envOpts=[] }) {
  const [form, setForm] = useState({
    ...
    os:             (initial as any).os           || detectOS(),
    browser:        (initial as any).browser      || detectBrowser(),
    impact:         (initial as any).impact        || "",
    evidence_url:   (initial as any).evidence_url  || "",
    ...
    <>
      ...
      <div className="form-row">
        <Field label="Severidade">
          <Select data-testid="select-bug-severidade" value={form.severity} onChange={v=>setForm(f=>({...f,severity:v}))} options={SEV_OPTS} />
        </Field>
        <Field label="Prioridade">
          <Select data-testid="select-bug-prioridade" value={form.priority} onChange={v=>setForm(f=>({...f,priority:v}))} options={PRIO_OPTS} />
        </Field>
      </div>
      <div className="form-row">
        <Field label="Status">
      ...
      <Field label="Descrição">
        <textarea data-testid="textarea-bug-descricao" value={form.description} onChange={set("description")} placeholder="Detalhes adicionais..." />
      </Field>
      ...
      <Field label="Impacto no negócio">
        <textarea value={form.impact} onChange={set("impact")} rows={2}
          placeholder="Ex: Novos clientes não podem ser cadastrados..."
          style={{width:"100%",padding:"8px",borderRadius:6,border:"1px solid var(--border)",fontSize:13,resize:"vertical",fontFamily:"inherit"}} />
      </Field>
      {bugId ? (
...
export default function Bugs() {
  ...
  const [confirm,        setConfirm]        = useState<Bug | null>(null);
        ...
        {!filtered.length ? <Empty icon="🐛" text="Nenhum bug encontrado." /> : (
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {paged.map(b => (
                  <tr data-testid={`bug-row-${b.id}`} key={b.id}>
